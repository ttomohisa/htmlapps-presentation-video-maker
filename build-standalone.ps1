...
$FfmpegVendorRoot = Join-Path $Root "vendor\ffmpeg-video-compressor-v1.6.0"
$FfmpegRuntimePath = Join-Path $FfmpegVendorRoot "browser-ffmpeg.js"
$FfmpegCoreGzipPath = Join-Path $FfmpegVendorRoot "ffmpeg.js.gz"
...
foreach ($requiredVendorPath in @($FfmpegRuntimePath, $FfmpegCoreGzipPath, $FfmpegWasmGzipPath, $FfmpegManifestPath)) {
  if (-not (Test-Path -LiteralPath $requiredVendorPath -PathType Leaf)) { throw "FFmpeg vendor asset not found: $requiredVendorPath" }
}
$ffmpegRuntimeJs = [System.IO.File]::ReadAllText($FfmpegRuntimePath, [System.Text.Encoding]::UTF8)
if ($ffmpegRuntimeJs -match '</script') { throw "Embedded FFmpeg runtime contains a closing script tag." }
$ffmpegCoreGzipBase64 = [Convert]::ToBase64String([System.IO.File]::ReadAllBytes($FfmpegCoreGzipPath))
...
$manifest = [ordered]@{
  ...
  ffmpeg = [ordered]@{
    ...
    coreJsGzipSha256 = (Get-Sha256FileHex $FfmpegCoreGzipPath)
    wasmGzipSha256 = (Get-Sha256FileHex $FfmpegWasmGzipPath)
  }
}
...
$replacements = [ordered]@{
  ...
  "__FFMPEG_RUNTIME_JS__" = $ffmpegRuntimeJs
  "__FFMPEG_CORE_JS_GZIP_BASE64__" = $ffmpegCoreGzipBase64
...
$replacementExpectedCounts = @{
  ...
  "__FFMPEG_RUNTIME_JS__" = 1
  "__FFMPEG_CORE_JS_GZIP_BASE64__" = 1
